"""
P2 Article - Multi-Class Baseline Pix2pix with Focal Loss (Variant 2)
...
class ExperimentConfig:
    ...
    def __init__(self, 
        ...
        self.focal_gamma = 2.0           # Focal loss focusing parameter
    ...
    def save_config(self):
        ...
        config_dict = {
            ...
            'variant_name': 'Multiclass_AttentionD_Focal',
            ...
            'beta_1': self.beta_1,
            'innovation': 'Phase-transitioning segmentation loss (Focal Loss)'
        }
...
def generator_loss(disc_generated_output, gen_output, target_onehot, 
                  class_weights, lambda_gan=1, lambda_seg=100, use_focal_alpha=True
                  ):
    """
    ...
    seg_loss = unified_focal_loss(target_onehot, gen_output, alpha=focal_alpha)
...
def train_experiment_with_metrics(config: ExperimentConfig):
    """
    'variant_name': 'Multiclass_Focal'
    Main training function for baseline multi-class pix2pix with focal loss
    ...
    print("="*70)
    print(f"Variant: {config.variant} (Baseline)")
    ...
    try:
        for epoch in range(config.epochs):
            ...
            new_lr = config.learning_rate * ((1-(1-0.5e-2)*(epoch / config.epochs)))  # Exponential decay from 2e-4 to 1e-6
            ...
            for val_paired, val_target in val_dataset:
                try:
                    ...
                    val_seg_loss = unified_focal_loss(val_target_onehot, val_pred, 
                                                           alpha=class_weights, exclude_class=exclude_class)
